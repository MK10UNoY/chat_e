<script lang="ts">
	...
	$effect(() => {
		canLogin = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email) && password.length > 0;
	});
</script>
					...
					<div class="grid gap-6">
						...
						<div class="grid gap-3">
							...
							<Input
								...
								class={password.length > 0 ? 'ring-2 ring-green-500 border-green-500' : ''}
								aria-invalid={password.length === 0}
							/>
						</div>
						<Button type="submit" class="w-full" disabled={!canLogin}>Login</Button>
					</div>
